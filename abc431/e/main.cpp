...
int main() {
  ...
  const auto solve = [&answers]() {
      ...
      for (s64 j = 0; j < w; ++j) {
        for (s64 k = 0; k < 4; ++k) {
          for (s64 l = 0; l < 4; ++l) {
            ...
            auto kc0 = make_tuple(i, j, k);
            auto [dh, dw] = diffs[l];
            auto kn = make_tuple(i + dh, j + dw, l);
            const s64 cost = (l == m[ss[i][j]][k]) ? 0 : 1;
            g[kc0].push_back(make_pair(cost, kn));
          }
        }
      }
    }
    ...
    pq.push(make_pair(0, make_tuple(0, 0, 0)));
    map<key, s64> ds;
    ds[make_tuple(0, 0, 0)] = 0;
    while (!pq.empty()) {
      auto [d, k] = pq.top();
      pq.pop();
    ...
    for (s64 k = 0; k < 4; ++k) {
      const auto key = make_tuple(h - 1, w - 1, k);
      if (ds.count(key) == 0) continue;
      s64 cost = ds[key];
      if (m[ss[h - 1][w - 1]][k] != 0) ++cost;
      ans = min(ans, cost);
    }
